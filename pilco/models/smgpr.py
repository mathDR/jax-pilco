        ...
        for i in range(self.num_outputs):
            ...
            self.models.append(
                gpflow.models.GPRFITC(
                    (data[0], data[1][:, i : i + 1]), kern, inducing_variable=Z
                )
            )

    def calculate_factorizations(self):
        batched_eye = tf.eye(
            self.num_induced_points, batch_shape=[self.num_outputs], dtype=float_type
        )
        ...
        Am = tf.linalg.cholesky(
            tf.matmul(V, V, transpose_b=True) + self.noise[:, None, None] * batched_eye
        )
        At = tf.matmul(L, Am)
        iAt = tf.linalg.triangular_solve(At, batched_eye)
        Y_ = tf.transpose(self.Y)[:, :, None]
        beta = tf.linalg.triangular_solve(
            L, tf.linalg.cholesky_solve(Am, (V / G[:, None]) @ Y_), adjoint=True
        )[:, :, 0]
