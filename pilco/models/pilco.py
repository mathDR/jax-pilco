...
class PILCO(objax.Module):
    def __init__(
        ...
        # TODO: add back SMGPR?
        self.dynamical_model = MGPR(data, trainable_likelihood_variance)
    ...
    def optimize_models(self, maxiter=1000, restarts=1):
        ...
        Optimize GP models
        """
        self.dynamical_model.optimize(maxiter=maxiter, restarts=restarts)
        # ToDo: only do this if verbosity is large enough
        lengthscales = {}
        variances = {}
        noises = {}
        for i, model in enumerate(self.dynamical_model.models):
    ...
    def propagate(self, m_x, s_x):
        ...
        M_dx, S_dx, C_dx = self.dynamical_model.predict_on_noisy_inputs(m, s)
        M_x = M_dx + m_x
