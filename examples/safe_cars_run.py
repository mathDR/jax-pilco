...
def safe_cars(seed=0):
    ...
    X1, Y1, _, _ = rollout(
        env, pilco=None, timesteps=T, verbose=True, random=True, render=False
    )
    for i in range(1, 5):
        X1_, Y1_, _, _ = rollout(
            env, pilco=None, timesteps=T, verbose=True, random=True, render=False
    ...
    R1 = LinearReward(state_dim=state_dim, W=jnp.array([1.0 * env.std[0], 0.0, 0.0, 0]))
    ...
    for model in pilco.mgpr.models:
        model.likelihood.transformed_variance.assign(softplus_inv(0.001))
    ...
    for rollouts in range(N):
        ...
        if overall_risk < th:
            X_new, Y_new, _, _ = rollout(
                env, pilco=pilco, timesteps=T, verbose=True, render=False
            )
            new_data = True
